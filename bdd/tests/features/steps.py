...
def entonces_obtengo_el_area_group1(step, esperado):
    ...
    assert float(esperado) == obtenido, 'El resultado esperado es' + esperado + ' y el obtenido es ' + obtenido  # noqa: E501
...
@step(u'Dado que ingreso la base menor "([^"]*)", base mayor "([^"]*)" y altura "([^"]*)"')  # noqa: E501
def dado_que_ingreso_la_base_menor_group1_base_mayor_group2_y_altura_group3(step, basema, baseme, altura):  # noqa: E501
    world.figs = Figuras()
    world.figs.calcular_area_trapecio(
        float(altura), float(basema), float(baseme))
